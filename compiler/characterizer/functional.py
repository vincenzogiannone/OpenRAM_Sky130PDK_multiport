...
class functional(simulation):
    ...
    def __init__(self, sram, spfile=None, corner=None, cycles=15, period=None, output_path=None):
        ...
        if OPTS.RF_mode == False:
            ...
            for port in range(OPTS.num_w_ports):
                debug.info(2, "wbl{0}:\t{1}".format(port, self.wbl_names[port]))
        ...
        if OPTS.RF_mode == False:
            ...
            self.write_functional_stimulus_multiport()
    ...
    def write_functional_stimulus_multiport(self):
        """ Writes SPICE stimulus. """
        self.stim_sp = "functional_stim.sp"
        temp_stim = "{0}/{1}".format(self.output_path, self.stim_sp)
        self.sf = open(temp_stim, "w")
        ...
        self.sf.write("\n\n* Important signals for debug\n")
        for port in range(OPTS.num_r_ports):
            self.sf.write("* rbl{0}:\t{1}\n".format(port, self.rbl_names[port]))
        for port in range(OPTS.num_w_ports):
            self.sf.write("* wbl{0}:\t{1}\n".format(port, self.wbl_names[port]))
        self.sf.write("* q:\t{0}\n".format(self.q_name))
        self.sf.write("* qbar:\t{0}\n".format(self.qbar_name))
        ...
        # Generate control signals
        self.sf.write("\n * Generation of control signals\n")
        self.csb_values_tmp = {0: []}
        self.web_values_tmp = {0: []}
        for x in range(len(self.csb_values[0])):
            if x<len(self.cycle_times):
                self.csb_values_tmp[0].append(0)
                self.web_values_tmp[0].append(0)
        self.stim.gen_pwl("CSB", self.cycle_times, self.csb_values_tmp[0], self.period, self.slew, 0.05)
        self.stim.gen_pwl("WEB", self.cycle_times, self.web_values_tmp[0], self.period, self.slew, 0.05)
        # Generate wmask bits
        for port in range(OPTS.num_w_ports):
            if self.write_size:
                self.sf.write("\n* Generation of wmask signals\n")
