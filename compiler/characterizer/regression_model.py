    ...
    def get_lib_values(self, load_slews):
        ...
        for load, slew in load_slews:
            ...
            if OPTS.RF_mode == False:
                ...
                port = 0
                port_data[0]['delay_lh'].append(sram_vals['rise_delay'])
                port_data[0]['delay_hl'].append(sram_vals['fall_delay'])
                port_data[0]['slew_lh'].append(sram_vals['rise_slew'])
                port_data[0]['slew_hl'].append(sram_vals['fall_slew'])
                
                port_data[0]['write1_power'].append(sram_vals['write1_power'])
                port_data[0]['write0_power'].append(sram_vals['write0_power'])
                port_data[0]['read1_power'].append(sram_vals['read1_power'])
                port_data[0]['read0_power'].append(sram_vals['read0_power'])
                
                # Disabled power not modeled. Copied from other power predictions
                port_data[0]['disabled_write1_power'].append(sram_vals['write1_power'])
                port_data[0]['disabled_write0_power'].append(sram_vals['write0_power'])
                port_data[0]['disabled_read1_power'].append(sram_vals['read1_power'])
                port_data[0]['disabled_read0_power'].append(sram_vals['read0_power'])
