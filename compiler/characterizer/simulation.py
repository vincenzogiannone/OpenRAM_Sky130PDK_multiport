...
class simulation():
    ...
    def set_stimulus_variables(self):
        ...
        if OPTS.RF_mode == False:
            ...
            self.data_value = {port: [] for port in range(OPTS.num_w_ports)}
            self.wmask_value = {port: [] for port in range(OPTS.num_w_ports)}
            self.spare_wen_value = {port: [] for port in range(OPTS.num_w_ports)}
        ...
        if OPTS.RF_mode == False:
            ...
            self.data_values = {port: [[] for bit in range(self.word_size + self.num_spare_cols)] for port in range(OPTS.num_w_ports)}
            self.wmask_values = {port: [[] for bit in range(self.num_wmasks)] for port in range(OPTS.num_w_ports)}
            self.spare_wen_values = {port: [[] for bit in range(self.num_spare_cols)] for port in range(OPTS.num_w_ports)}
    ...
    def add_control_one_port(self, port, op):
        ...
        # If port is in both lists, add rw control signal. Condition indicates its a RW port.
            ...
            self.csb_values[0].append(0)
            self.web_values[0].append(0)
    ...
    def gen_pin_names(self, port_signal_names, port_info, abits, dbits):
        ...
        #Control signals not finalized.
            ...
            for write_input in range(OPTS.num_w_ports):
                for i in range(dbits):
                    pin_names.append("{0}{1}_{2}".format(din_name, write_input, i))
                    
            for port in range(OPTS.num_r_ports):
                for i in range(abits):
                    pin_names.append("read_addr{0}_{1}".format(port, i))
                    
            for port in range(OPTS.num_w_ports):
                for i in range(abits):
                    pin_names.append("write_addr{0}_{1}".format(port, i))
            pin_names.append("CSB")
            pin_names.append("WEB")
            pin_names.append("clk")
            if self.write_size:
                for port in range(OPTS.num_w_ports):
                    for bit in range(self.num_wmasks):
                        pin_names.append("WMASK{0}_{1}".format(port, bit))
            ...
            for read_output in range(OPTS.num_r_ports):
                for i in range(dbits):
                    pin_names.append("{0}{1}_{2}".format(dout_name, read_output, i))
    ...
    def set_internal_spice_names(self):
        ...
        if OPTS.RF_mode == False:
            ...
            if not OPTS.use_pex or (OPTS.use_pex and OPTS.pex_exe[0] == "calibre"):
                ...
                rbl0_name_port = self.get_bl_name_multiport(self.graph.all_paths)
                port_pos = -1 - len(str(column_addr)) - len(str(port))
    ...
    def get_bl_name_multiport(self, paths):
