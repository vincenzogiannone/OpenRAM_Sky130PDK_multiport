...
class lib:
    ...
    def characterize(self):
        ...
        # Loop over all ports.
            ...
            self.write_data_bus(port = None)
            self.write_addr_bus(port = None)
            if self.sram.write_size:
                self.write_wmask_bus(port = None)
            self.write_control_pins(port = None)
            self.write_clk_timing_power(port = None)
    ...
    def write_addr_bus(self, port):
        ...
        if OPTS.RF_mode == False:
            ...
            self.lib.write("        pin(addr[{}:0])".format(self.sram.addr_size-1))
        self.lib.write("{\n")
        ...
        # Open for write and truncate to not conflict with a previous run using the same name
            ...
            for port in range(OPTS.num_r_ports):
                ...
                datasheet.write("{0},{1},{2},{3},{4},{5},{6},{7},{8},".format(
                        ...
                        max(list(map(round_time,self.char_port_results[0]["delay_lh"]))),

                        min(list(map(round_time,self.char_port_results[0]["delay_hl"]))),
                        max(list(map(round_time,self.char_port_results[0]["delay_hl"]))),

                        min(list(map(round_time,self.char_port_results[0]["slew_lh"]))),
                        max(list(map(round_time,self.char_port_results[0]["slew_lh"]))),

                        min(list(map(round_time,self.char_port_results[0]["slew_hl"]))),
                        max(list(map(round_time,self.char_port_results[0]["slew_hl"])))
                        ))
    ...
    def write_power_datasheet(self, datasheet):
        ...
        # write leakage power
            ...
            for port in range(OPTS.num_r_ports):
                web_name = " & !web"
                name = "!csb & clk_{0}".format(web_name)
                read_write = 'Read'
    ...
    def write_model_params(self, datasheet, time):
        ...
        if OPTS.RF_mode == False:
            ...
            for port in range(OPTS.num_r_ports):
                read1_power = np.mean(self.char_port_results[0]["read1_power"])
                read0_power = np.mean(self.char_port_results[0]["read0_power"])
                datasheet.write("{0},{1},".format('read_rise_power_{}'.format(port), read1_power))
            #FIXME: should be read_fall_power
                datasheet.write("{0},{1},".format('read_fall_power_{}'.format(port), read0_power))
