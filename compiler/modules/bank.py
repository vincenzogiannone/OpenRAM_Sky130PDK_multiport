...
class bank(design.design):
    ...
    def create_layout(self):
        ...
        if OPTS.RF_mode == False:
            self.predecoder_top = self.port_address[-1].predecoder_height + self.port_address_inst[-1].by()
            self.predecoder_bottom = self.port_address_inst[-1].by()
        else:
            self.predecoder_top = self.port_address[-1].predecoder_height + self.port_address_inst.by()
            self.predecoder_bottom = self.port_address_inst.by()
    ...
    def route_bitlines(self, port):
        ...
        if OPTS.RF_mode == False:
            ...
            self.route_port_data_in(port=None)
            self.route_port_data_out(port=None)
            self.route_port_data_to_bitcell_array(port=None)
    ...
    def compute_instance_offsets(self):
        ...
        if OPTS.RF_mode == False:
            ...
            self.port_data_offsets = []
            self.port_address_offsets = []
            self.column_decoder_offsets = [None] 
    ...
    def compute_instance_port0_offsets(self):
        ...
        #self.port_data_offsets = vector(0, -self.port_data_inst.height-self.m2_gap)
        if OPTS.RF_mode == False:
        ...
        x_offset = self.central_bus_width[port] + self.port_address[-1].wordline_driver_array.width
        ...
        if OPTS.RF_mode == False:
            ...
            self.column_decoder_offsets = vector(-x_offset, -y_offset)
        # Bank select gets placed below the column decoder (x_offset doesn't change)
        if self.col_addr_size > 0:
            y_offset = min(self.column_decoder_offsets.y, self.port_data.column_mux_offset.y)
        else:
            y_offset = self.port_address_offsets[-1].y
        if self.num_banks > 1:
    ...
    def compute_instance_port1_offsets(self):
        ...
        # by placing 1 1/4 a cell pitch down because both power connections and inputs/outputs
        # may be routed in M3 or M4
        x_offset = self.bitcell_array_right  + self.central_bus_width[port] + self.port_address[-1].wordline_driver_array.width
        if self.col_addr_size > 0:
            x_offset += self.column_decoder.width + self.col_addr_bus_width
    ...
    def compute_sizes(self):
        ...
        if OPTS.RF_mode == False:
            ...
            self.row_addr_size = ceil(log(OPTS.num_all_ports*self.num_rows, 2))
        ...
        # A space for wells or jogging m2
            ...
            self.input_control_signals.append(["p_en_bar"])
        ...
        # These will be outputs of the gaters if this is multibank, if not, normal signals.
            self.control_signals = []
            for port in range(len(self.input_control_signals)):
                if self.num_banks > 1:
                    self.control_signals.append(["gated_" + str for str in self.input_control_signals[port]])
                else:
                    self.control_signals.append(self.input_control_signals[port])
    ...
    def add_modules(self):
        ...
        if OPTS.RF_mode == False:
            ...
            self.port_data = factory.create(module_type="port_data",
                                          sram_config=self.sram_config,
                                          port=None,
                                          bit_offsets=None)
            self.add_mod(self.port_data)
        if(self.num_banks > 1):
    ...
    def create_bitcell_array(self):
        ...
        if OPTS.RF_mode == False:
            ...
            temp = self.bitcell_array.get_bitline_names()
            temp.extend(self.bitcell_array.get_wordline_names())
        temp.append("vdd")
        temp.append("gnd")
    ...
    def create_port_data(self):
        ...
        if OPTS.RF_mode == False:
            ...
            self.port_data_inst = []
            self.port_data_inst=self.add_inst(name="port_data",
                                              mod=self.port_data)
            tmp = []            
            for col in range(self.num_cols):
                for port in range(OPTS.num_r_ports):
                    tmp.append("rbl{0}_{1}".format(port, col))
                for port in range(OPTS.num_w_ports):
                    tmp.append("wbl{0}_{1}".format(port, col))
            for bit in range(self.word_size + self.num_spare_cols):
                for port in range(OPTS.num_r_ports):
                    tmp.append("dout{0}_{1}".format(port, bit))
            for bit in range(self.word_size + self.num_spare_cols):
                for port in range(OPTS.num_w_ports):
                    tmp.append("din{0}_{1}".format(port, bit))
            # Will be empty if no col addr lines
            sel_names = ["sel{0}_{1}".format(port, x) for x in range(self.num_col_addr_lines)]
            tmp.extend(sel_names)
            tmp.append("p_en_bar{0}".format(port))
            for port in range(OPTS.num_w_ports):
                tmp.append("w_en{0}".format(port))
                #for bit in range(self.num_wmasks):
                 #   temp.append("bank_wmask{0}_{1}".format(port, bit))#FIXME
                #for bit in range(self.num_spare_cols):
                 #   temp.append("bank_spare_wen{0}_{1}".format(port, bit))
            tmp.extend(["vdd", "gnd"])
            self.connect_inst(tmp)
    ...
    def place_port_data(self, offsets):
        ...
        if OPTS.RF_mode == False:
            ...
            self.port_data_inst.place(offset=offsets, mirror="R0") 
    ...
    def create_port_address(self):
        ...
        if OPTS.RF_mode == False:
            ...
            self.port_address_inst = self.add_inst(name="port_address",
                                                   mod=self.port_address[-1])
            temp = []
            for bit in range(self.row_addr_size):
                temp.append("addr{}".format(bit + self.col_addr_size))
            temp.append("wl_en")
        ...
        # The address flop and decoder are aligned in the x coord.
            ...
            for port in self.all_ports:
                if port % 2:
                    ...
                    mirror = "R0" 
                self.port_address_inst[-1].place(offset=offsets[port], mirror=mirror)
        else:
            self.port_address_inst.place(offset = offsets, mirror = "R0")
    ...
    def create_column_decoder(self):
        ...
        if OPTS.RF_mode == False:
            ...
            self.column_decoder_inst = self.add_inst(name="col_address_decoder",
                                                           mod=self.column_decoder)

            temp = []
            for bit in range(self.col_addr_size):
                temp.append("addr{0}".format(bit))
            for bit in range(self.num_col_addr_lines):
                temp.append("sel{0}".format(bit))
            temp.extend(["vdd", "gnd"])
            self.connect_inst(temp)
    ...
    def place_column_decoder(self, offsets):
        ...
        if OPTS.RF_mode == False:
            ...
            mirror = "R0"
            self.column_decoder_inst.place(offset=offsets, mirror=mirror)
    ...
    def route_central_bus(self):
        ...
        # 2 pitches on the right for vias/jogs to access the inputs
            control_bus_offset = vector(-self.m3_pitch * self.num_control_lines[0] - 2 * self.m3_pitch, self.min_y_offset)
        # The control bus is routed up to two pitches below the bitcell array
            control_bus_length = self.port_data_inst.uy() - self.min_y_offset
            self.bus_pins[0] = self.create_bus(layer="m2",
                                               offset=control_bus_offset,
                                               names=self.control_signals[0],
                                               length=control_bus_length,
            ...
            self.copy_layout_pin(self.port_address_inst, "wl_en", self.prefix + "wl_en")
        # Port 1
            # The other control bus is routed up to two pitches above the bitcell array
            control_bus_length = self.max_y_offset - self.port_data_inst.by()
            control_bus_offset = vector(self.bitcell_array_right + 2.5 * self.m3_pitch,
                                        self.max_y_offset - control_bus_length)
            # The bus for the right port is reversed so that the rbl_wl is closest to the array
            self.bus_pins[1] = self.create_bus(layer="m2",
                                               offset=control_bus_offset,
                                               names=list((self.control_signals[1])),
                                               length=control_bus_length,
                                               vertical=True,
                                               make_pins=(self.num_banks==1),
                                               pitch=self.m3_pitch)
    ...
    def route_port_data_to_bitcell_array(self, port):
        ...
        # Connect the replica bitlines
            ...
            inst2 = self.port_data_inst
            inst1 = self.bitcell_array_inst
            inst1_rbl_name = [x for x in self.bitcell_array.get_read_bitline_names(port)]
            inst1_wbl_name = [x for x in self.bitcell_array.get_write_bitline_names(port)]
            inst2_rbl_name = []
            inst2_wbl_name = []
            for col in range(self.num_cols):
                for port in range(OPTS.num_r_ports):
                    inst2_rbl_name.append(inst2.mod.get_rbl_names()[port] + "_{}".format(col))
                for port in range(OPTS.num_w_ports):    
                    inst2_wbl_name.append(inst2.mod.get_wbl_names()[port] + "_{}".format(col))
            for col in range(self.num_spare_cols):
                for port in range(OPTS.num_r_ports):
            ...
            self.connect_bitlines_multiport(inst1=inst1,
                                  ...
                                  inst1_wbl_name=inst1_wbl_name,
                                  inst2_rbl_name=inst2_rbl_name,
                                  inst2_wbl_name=inst2_wbl_name)
        ...
        if OPTS.RF_mode == False:
            ...
            for bit in range(self.word_size + self.num_spare_cols):
                for x in range(OPTS.num_r_ports):
                    data_pin = self.port_data_inst.get_pin("dout{0}_{1}".format(x,bit))
    ...
    def route_port_address_in(self, port):
        ...
        # Create inputs for the row address lines
            ...
            for row in range(self.row_addr_size):
                ...
                addr_name = "addr{0}".format(addr_idx)
                self.copy_layout_pin(self.port_address_inst, decoder_name, addr_name)
        ...
        if OPTS.RF_mode == False:
            ...
            for row in range(self.word_size + self.num_spare_cols):
                for port in range(OPTS.num_w_ports):
                    ...
                    din_name = "din{0}_{1}".format(port, row)
                    self.copy_layout_pin(self.port_data_inst, data_name, din_name)

            if self.write_size:
                for row in range(self.num_wmasks):
                    wmask_name = "bank_wmask_{}".format(row)
                    bank_wmask_name = "bank_wmask{0}_{1}".format(port, row)
                    self.copy_layout_pin(self.port_data_inst, wmask_name, bank_wmask_name)

            for col in range(self.num_spare_cols):
                sparecol_name = "bank_spare_wen{}".format(col)
                bank_sparecol_name = "bank_spare_wen{0}_{1}".format(port, col)
                self.copy_layout_pin(self.port_data_inst, sparecol_name, bank_sparecol_name)
    ...
    def connect_bitlines_multiport(self, inst1, inst2,
                         inst1_rbl_name, inst1_wbl_name,
                         inst2_rbl_name, inst2_wbl_name):
        """
        Connect the rbls and wbls of two modules.
        """
        for (name1, name2) in zip(inst1_rbl_name, inst2_rbl_name):
            self.connect_bitline(inst1, inst2, name1, name2)
        for (name1, name2) in zip(inst1_wbl_name, inst2_wbl_name):
            self.connect_bitline(inst1, inst2, name1, name2)
    ...
    def route_port_address_out(self, port, side="left"):
        ...
        if OPTS.RF_mode == False:
            ...
            for x in range(self.num_rows):
                for port in range(OPTS.num_r_ports):
                    driver_names.append("rwl{}_{}".format(port, x))
                for port in range(OPTS.num_w_ports):
                    driver_names.append("wwl{}_{}".format(port, x))
                
            for (driver_name, array_name) in zip(driver_names, self.bitcell_array.get_wordline_names()):
            # The mid guarantees we exit the input cell to the right.
                driver_wl_pin = self.port_address_inst.get_pin(driver_name)
                if side == "left":
                    driver_wl_pos = driver_wl_pin.rc()
                else:
                    driver_wl_pos = driver_wl_pin.lc()
                ...
                if side == "left":
                    bitcell_wl_pos = bitcell_wl_pin.lc()
                    port_address_pos = self.port_address_inst.rx()
                    bitcell_array_pos = self.bitcell_array_inst.lx()
                else:
                    bitcell_wl_pos = bitcell_wl_pin.rc()
                    port_address_pos = self.port_address_inst.lx()
                    bitcell_array_pos = self.bitcell_array_inst.rx()
                #print(driver_wl_pos, bitcell_wl_pin, bitcell_wl_pos, port_address_pos, bitcell_array_pos)
    ...
    def route_column_address_lines(self, port):
        ...
        if OPTS.RF_mode == False:
            ...
            # The Address LSB
                self.copy_layout_pin(self.column_decoder_inst, "A", "addr0")
                ...
                for i in range(self.col_addr_size):
                    decoder_name = "in_{}".format(i)
                    addr_name = "addr{0}".format(i)
                    self.copy_layout_pin(self.column_decoder_inst, decoder_name, addr_name)

            offset = self.column_decoder_inst.ll() - vector((self.num_col_addr_lines + 1) * pitch, 0)

            decode_pins = [self.column_decoder_inst.get_pin(x) for x in decode_names]

            sel_names = ["sel_{}".format(x) for x in range(self.num_col_addr_lines)]
            column_mux_pins = [self.port_data_inst.get_pin(x) for x in sel_names]
    ...
    def route_control_lines(self, port):
        ...
        if OPTS.RF_mode == False:
            ...
            connection = []
            connection.append((self.prefix + "w_en",
                               self.port_data_inst.get_pin("w_en")))
                               
            connection.append((self.prefix + "p_en_bar",
                               self.port_data_inst.get_pin("p_en_bar")))
    ...
    def graph_exclude_precharge(self):
        ...
        if OPTS.RF_mode == False:
            ...
            self.port_data.graph_exclude_precharge()
    ...
    def graph_exclude_column_mux(self, column_include_num, port):
        ...
        if OPTS.RF_mode == False:
            ...
            self.port_data.graph_exclude_column_mux(column_include_num)
    ...
    def graph_clear_column_mux(self, port):
        ...
        if OPTS.RF_mode == False:
            ...
            self.port_data.graph_clear_column_mux()
