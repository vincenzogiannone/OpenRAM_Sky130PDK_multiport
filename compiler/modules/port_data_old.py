...
class port_data(design.design):
    ...
    def add_pins(self):
        ...
        # Will be empty if no col addr lines
            ...
            for port in range(OPTS.num_all_ports):
                self.add_pin("rbl_bl{0}".format(port), "INOUT")
            for bit in range(self.num_cols):
            ...
            if self.port in self.write_ports:
                for bit in range(self.word_size + self.num_spare_cols):
                    for x in range(OPTS.num_w_ports):
                        self.add_pin("din_{0}_{1}".format(x, bit), "INPUT")
        # Will be empty if no col addr lines
            sel_names = ["sel_{}".format(x) for x in range(self.num_col_addr_lines)]
            for pin_name in sel_names:
                self.add_pin(pin_name, "INPUT")
    ...
    def add_modules(self):
        ...
        if OPTS.RF_mode == False:
            ...
            self.precharge_array = factory.create(module_type="precharge_array_multiport",
                                                  columns=self.num_cols + self.num_spare_cols + 1,
                                                  offsets=precharge_bit_offsets,
                                                  bitcell_rbl=self.rbl_names[self.port],
                                                  column_offset=self.port - 1)                                          
    ...
    def create_precharge_array(self):
        ...
        # Use left BLs for RBL
        if self.port==0:
            temp.append("rbl_read_bl0")
            temp.append("rbl_read_bl1")
        for bit in range(self.num_cols):
            for port in range(OPTS.num_r_ports):
                temp.append("rbl{0}_{1}".format(port, bit))
        for bit in range(self.num_spare_cols):
            for port in range(OPTS.num_r_ports):
                temp.append("spare_rbl{0}_{1}".format(port, bit))
        # Use right BLs for RBL
        if self.port==1:
            temp.append("rbl_read_bl0")
            temp.append("rbl_read_bl1")  
        ...
        for bit in range(self.num_cols):
            for port in range(OPTS.num_r_ports):
                temp.append("rbl{0}_{1}".format(port, bit))    
        temp.append("rbl_read_bl0")
        temp.append("rbl_read_bl1")  #FIXME
    ...
    def create_column_mux_array(self):
        ...
        if OPTS.RF_mode == False:
            ...
            self.column_mux_array_inst = self.add_inst(name="column_mux_array{}".format(self.port),
                                                       mod=self.column_mux_array_multiport)
        temp = []
    ...
    def create_sense_amp_array(self):
        ...
        if OPTS.RF_mode == False:
            ...
            self.sense_amp_array_inst = self.add_inst(name="sense_amp_array{}".format(self.port),
                                                      mod=self.sense_amp_array_multiport)
    ...
    def create_write_driver_array(self):
        ...
        if OPTS.num_all_ports == OPTS.num_r_ports:
            for bit in range(self.word_size):
                if (self.words_per_row == 1):
                    temp.append("bl_{0}".format(bit))
                    temp.append("br_{0}".format(bit))
    ...
    def route_bitline_pins(self):
        ...
        # Copy bitlines of spare columns
            ...
            # Connect one bitline to the RBL and offset the indices for the other BLs
            if self.port==0:
                self.copy_layout_pin(self.precharge_array_inst, "rbl0_0", "rbl_bl")
                self.copy_layout_pin(self.precharge_array_inst, "rbl1_0", "rbl_bl")
                bit_offset=1
            elif self.port==1:
                self.copy_layout_pin(self.precharge_array_inst, "rbl0_{}".format(self.num_cols + self.num_spare_cols), "rbl_bl")
                self.copy_layout_pin(self.precharge_array_inst, "rbl1_{}".format(self.num_cols + self.num_spare_cols), "rbl_bl")
                bit_offset=0
            else:
                bit_offset=0
            ...
            for bit in range(self.num_cols):
                if self.precharge_array_inst:
                    self.copy_layout_pin(self.precharge_array_inst,
                                         "rbl0_{}".format(bit + bit_offset),
                                         "rbl0_{}".format(bit))
                    self.copy_layout_pin(self.precharge_array_inst,
                                         "rbl1_{}".format(bit + bit_offset),
                                         "rbl1_{}".format(bit))
                else:
                    debug.error("Didn't find precharge array.")
    ...
    def _group_bitline_instances(self, inst1, inst2, num_bits,
        ...
        # since they don't overlap, we can just check the bottom y coordinate.
            ...
            if "precharge_array_multiport" in str(inst1.mod) and not "write_driver_array" in str(inst2.mod):
                inst_group = namedtuple('InstanceGroup', ('inst', 'bls_template',
                ...
                inst2_group = inst_group(inst2, inst2_bls_template,
                                         ...
                                         inst2_start_bit)
            elif "precharge_array_multiport" in str(inst2.mod) and not "write_driver_array" in str(inst1.mod):
                inst1_group = inst_group(inst1, inst1_bls_template,
                ...
                inst2_group = inst_group(inst2, inst2_bls_template,
                                         ...
                                         inst2_start_bit)
            elif "write_driver_array" in str(inst1.mod) and not "precharge_array_multiport" in str(inst2.mod):
                ...
                inst2_group = inst_group(inst2, inst2_bls_template,
                                         ...
                                         inst2.mod.get_wbl_name(),
                                         inst2_start_bit)
            elif "write_driver_array" in str(inst2.mod) and not "precharge_array_multiport" in str(inst1.mod):
                inst1_group = inst_group(inst1, inst1_bls_template,
                                         inst1.mod.get_rbl_name(),
                                         inst1.mod.get_wbl_name(),
                                         inst1_start_bit)
 ...
 #FIXME            #self.create_horizontal_channel_route(route_map, offset, self.m1_stack)
    ...
    def connect_bitlines(self, inst1, inst2, num_bits,
        ...
        if OPTS.RF_mode == False: 
            ...
            for col in range(num_bits):
                bot_rbl_pin, bot_wbl_pin = self._get_bitline_pins(bot_inst_group, col)
                top_rbl_pin, top_wbl_pin = self._get_bitline_pins(top_inst_group, col)
                bot_rbl, bot_wbl = bot_rbl_pin.uc(), bot_wbl_pin.uc()
                top_rbl, top_wbl = top_rbl_pin.bc(), top_wbl_pin.bc()

                layer_pitch = getattr(self, "{}_pitch".format(top_bl_pin.layer))
                self.add_zjog(bot_rbl_pin.layer, bot_rbl, top_rbl, "V", fixed_offset=top_rbl_pin.by() - layer_pitch)
                self.add_zjog(bot_wbl_pin.layer, bot_wbl, top_wbl, "V", fixed_offset=top_rbl_pin.by() - 2 * layer_pitch)
