...
class hierarchical_decoder(design.design):
    ...
    def add_modules(self):
        ...
        # self.add_mod(self.and4)
            ...
            self.dec_cell2_2r1w = factory.create(module_type="dec_cell2_2r1w",
                                                 height=self.cell_height)
            self.add_mod(self.dec_cell2_2r1w)
                                       
            self.dec_cell3_2r1w = factory.create(module_type="dec_cell3_2r1w",
                                                 height=self.cell_height)
            self.add_mod(self.dec_cell3_2r1w)
        self.add_decoders()
                ...
                for i in range(len(self.predec_groups[0])):
                    for j in range(len(self.predec_groups[1])):
                        ...
                        if (output < self.num_outputs/OPTS.num_all_ports):
                            ...
                            pins =["out0_{0}".format(i),
                                   "out0_{0}".format(j + len(self.predec_groups[0])),
                                   "out1_{0}".format(i),
                                   "out1_{0}".format(j + len(self.predec_groups[0])),
                                   "out2_{0}".format(i),
                                   "out2_{0}".format(j + len(self.predec_groups[0])),
                                   "decode0_{0}".format(output),
                                   "decode1_{0}".format(output),
                    ...
                    for j in range(len(self.predec_groups[1])):
                        for k in range(len(self.predec_groups[2])):
                            ...
                            if (output < self.num_outputs/OPTS.num_all_ports):
                                ...
                                pins = ["out0_{0}".format(i),
                                        "out0_{0}".format(j + len(self.predec_groups[0])),
                                        "out0_{0}".format(k + len(self.predec_groups[0]) + len(self.predec_groups[1])),
                                        "out1_{0}".format(i),
                                        "out1_{0}".format(j + len(self.predec_groups[0])),
                                        "out1_{0}".format(k + len(self.predec_groups[0]) + len(self.predec_groups[1])),
                                        "out2_{0}".format(i),
                                        "out2_{0}".format(j + len(self.predec_groups[0])),
                                        "out2_{0}".format(k + len(self.predec_groups[0]) + len(self.predec_groups[1])),
                                        "decode0_{0}".format(output),
                                        "decode1_{0}".format(output),
                                        "decode2_{0}".format(output),
                                        "vdd", "gnd"]
    ...
    def place_decoder_and_array(self):
        ...
        # FIXME: why this correct offset?)
            ...
            if (self.num_inputs == 4 or self.num_inputs == 5):
                ...
                self.place_and_array(mod=self.dec_cell3_2r1w)

    def place_and_array(self, mod):
        """
        ...
        if OPTS.RF_mode == False:
            ...
            for row in range(int(self.num_outputs/OPTS.num_all_ports)):
                ...
                x_off = self.internal_routing_width
                self.dec_cell_inst[row].place(offset=vector(x_off, y_off),
                                              mirror=mirror)
        ...
        if OPTS.RF_mode == False:
            ...
            for row in range(int(self.num_outputs/OPTS.num_all_ports)):
                for port in range(OPTS.num_all_ports):
                    ...
                    self.copy_layout_pin(dec_cell_inst, "OUT{}".format(port), "decode{0}_{1}".format(port, row))
            ...
            for row in range(0, int(self.num_outputs/OPTS.num_all_ports)):
                for pin_name in ["vdd", "gnd"]:
                    ...
                    if OPTS.RF_mode == False:
                        ...
                        supply_pin = self.dec_cell_inst[row].get_pin(pin_name)
