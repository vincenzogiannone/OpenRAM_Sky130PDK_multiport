...
class sram_1bank(sram_base):
    ...
    def place_instances(self):
        ...
        if OPTS.RF_mode == False:
            ...
            self.data_bus_size = 0
            self.col_addr_bus_size = 0
            self.col_addr_bus_size = self.col_addr_size * self.m4_nonpref_pitch
            self.data_bus_size += self.num_wmasks + self.word_size
            # Convert to length
            self.data_bus_size *= self.m4_nonpref_pitch
            # Add the gap in unit length
            self.data_bus_size += self.data_bus_gap
    ...
    def place_control(self):
        ...
        # using the control_logic_center value.
            ...
            self.control_pos = vector(-self.control_logic_insts.width - 2 * self.m2_pitch, self.bank.bank_array_ll.y - self.control_logic_insts.mod.control_logic_center.y)
            self.control_logic_insts.place(self.control_pos)
    ...
    def place_dffs(self):
        ...
        # the control logic signals
            ...
            # the control logic signals
                y_offset = max(self.bank.height + self.data_bus_size + self.dff.height,
            ...
            port = 0
        # Add the col address flops below the bank to the right of the control logic
            x_offset = self.control_logic_insts.rx() + self.dff.width
        # Place it a data bus below the x-axis, but at least as low as the control logic to not block
        # the control logic signals
            y_offset = min(- self.data_bus_size - self.dff.height,
                           self.control_logic_insts.by())
            if self.col_addr_dff:
                self.col_addr_pos[port] = vector(x_offset,
                                             y_offset)
                self.col_addr_dff_insts[port].place(self.col_addr_pos[port])
            ...
            # Add the data flops below the write mask flops.
                self.data_pos[port] = vector(x_offset,
                                             y_offset)
                self.data_dff_insts.place(self.data_pos[port])
                x_offset = self.data_dff_insts.rx()

            # Add spare write enable flops to the right of data flops since the spare columns
            # will be on the right
                if self.num_spare_cols:
                    self.spare_wen_pos = vector(x_offset,
                                                      y_offset)
                    self.spare_wen_dff_insts.place(self.spare_wen_pos)
                    x_offset = self.spare_wen_dff_insts.rx()
    ...
    def add_layout_pins(self, add_vias=True):
        ...
        if OPTS.RF_mode == False:
            ...
            for signal in self.control_logic_inputs:
                self.add_io_pin(self.control_logic_insts,
                                signal,
                                signal, 
                                start_layer=pin_layer)
            ...
            for bit in range(self.word_size + self.num_spare_cols):
                for port in range(OPTS.num_w_ports):
                    self.add_io_pin(self.data_dff_insts,
                                    "din_{}".format(bit),
                                    "din{0}[{1}]".format(port, bit),
                                    start_layer=pin_layer)
    ...
    def route_dffs(self, add_routes=True):
        if OPTS.RF_mode == False:
            ...
            for port in range(OPTS.num_w_ports):
                self.route_dff(port, add_routes=False)
    ...
    def route_data_dffs(self, port, add_routes):
        ...
        # spare wen dff
            ...
            # synchronized inputs from data dff
                dff_names = ["dout_{}".format(x) for x in range(self.word_size + self.num_spare_cols)]
                dff_pins = [self.data_dff_insts.get_pin(x) for x in dff_names]
                bank_names = ["din{0}_{1}".format(port, x) for x in range(self.word_size + self.num_spare_cols)]
                bank_pins = [self.bank_inst.get_pin(x) for x in bank_names]
                route_map.extend(list(zip(bank_pins, dff_pins)))
        ...
        # spare wen dff
            ...
            # This layer stack must be different than the column addr dff layer stack
                ...
                y_offset = y_bottom - self.data_bus_size + 2 * self.m3_pitch
                offset = vector(self.control_logic_insts.rx() + self.dff.width,
                                    y_offset)
                cr = channel_route(netlist=route_map,
                                   offset=offset,
                                   layer_stack=layer_stack,
                ...
                if add_routes:
                   ...
                   self.data_bus_size = max(cr.height, self.col_addr_bus_size) + self.data_bus_gap
    ...
    def route_clk(self):
        ...
        # This is the actual input to the SRAM
            ...
            for x in range(OPTS.num_w_ports):                  
                data_dff_clk_pin = self.data_dff_insts.get_pin("clk")
                data_dff_clk_pos = data_dff_clk_pin.center()
                mid_pos = vector(clk_steiner_pos.x, data_dff_clk_pos.y)
                # In some designs, the steiner via will be too close to the mid_pos via
                # so make the wire as wide as the contacts
    ...
    def route_control_logic(self):
        ...
        if OPTS.RF_mode == False:
            ...
            for signal in self.control_logic_outputs:
                if "clk" in signal:
                    continue 
                src_pin = self.control_logic_insts.get_pin(signal)
                dest_pin = self.bank_inst.get_pin(signal)
                self.connect_vbus(src_pin, dest_pin)
    ...
    def graph_exclude_data_dff(self):
        ...
        if OPTS.RF_mode == False:
            ...
            self.graph_inst_exclude.add(self.data_dff_insts)
            if self.write_size:
                self.graph_inst_exclude.add(self.wmask_dff_insts)
            if self.num_spare_cols:
                self.graph_inst_exclude.add(self.spare_wen_dff_insts)
    ...
    def graph_exclude_ctrl_dffs(self):
        ...
        if OPTS.RF_mode == False:
            ...
            self.control_logic_insts.mod.graph_exclude_dffs()
