...
class sram_base(design, verilog, lef):
    ...
    def add_pins(self):
        ...
        # add the optional write mask pins
            ...
            self.control_logic_outputs.extend(self.control_logic_multiport.get_outputs())
            self.add_pin("csb", "INPUT")
            self.add_pin("web", "INPUT")
    ...
    def add_global_pex_labels(self):
        ...
        # add pex labels for control logic
            ...
            for bank_num in range(len(self.bank_insts)):
                ...
                Q_offset = pex_data[1] # offset of storage relative to bank
                Q_bar_offset = pex_data[2] # offset of storage relative to bank
                rbl0_offsets = pex_data[3]
                rbl1_offsets = pex_data[4]
                wbl0_offsets = pex_data[5]
                rbl0_meta = pex_data[6]
                rbl1_meta = pex_data[7]
                wbl0_meta = pex_data[8]
                ...
                storage_layer_name = "m1"
                bitline_layer_name = self.bitcell.get_pin("rbl0").layer
                ...
                for cell in range(len(rbl0_offsets)):
                    col = rbl0_meta[cell][0][2]
                    for bitline in range(len(rbl0_offsets[cell])):
                        bitline_location = [float(bank_offset[cell][0]) + rbl0_offsets[cell][bitline][0],
                                            float(bank_offset[cell][1]) + rbl0_offsets[cell][bitline][1]]
                        rbl0.append([bitline_location, rbl0_meta[cell][bitline][3], col])

                for cell in range(len(rbl1_offsets)):
                    col = rbl1_meta[cell][0][2]
                    for bitline in range(len(rbl1_offsets[cell])):
                        bitline_location = [float(bank_offset[cell][0]) + rbl1_offsets[cell][bitline][0],
                                            float(bank_offset[cell][1]) + rbl1_offsets[cell][bitline][1]]
                        rbl1.append([bitline_location, rbl1_meta[cell][bitline][3], col])
                        
                for cell in range(len(wbl0_offsets)):
                    col = wbl0_meta[cell][0][2]
                    for bitline in range(len(wbl0_offsets[cell])):
                        bitline_location = [float(bank_offset[cell][0]) + wbl0_offsets[cell][bitline][0],
                                            float(bank_offset[cell][1]) + wbl0_offsets[cell][bitline][1]]
                        wbl0.append([bitline_location, wbl0_meta[cell][bitline][3], col])

                for i in range(len(rbl0)):
                    self.add_layout_pin_rect_center("rbl0_{0}_{1}".format(rbl0[i][1], rbl0[i][2]),
                                                    bitline_layer_name, rbl0[i][0])

                for i in range(len(rbl1)):
                    self.add_layout_pin_rect_center("rbl1_{0}_{1}".format(rbl1[i][1], rbl1[i][2]),
                                                    bitline_layer_name, rbl1[i][0])
                
                for i in range(len(wbl0)):
                    self.add_layout_pin_rect_center("wbl0_{0}_{1}".format(wbl0[i][1], wbl0[i][2]),
                                                    bitline_layer_name, wbl0[i][0])

        # add pex labels for control logic
            instance = self.control_logic_insts
            control_logic_offset = instance.offset
            for output in instance.mod.output_list:
                pin = instance.mod.get_pin(output)
                pin.transform([0, 0], instance.mirror, instance.rotate)
    ...
    def route_supplies(self, bbox=None):
        ...
        for pin_name in ["vdd", "gnd"]:
            for inst in self.insts:
                ...
                #print(inst, pin_name, self.ext_supply[pin_name], self.copy_power_pins(inst, pin_name, self.ext_supply[pin_name])) 
    ...
    def route_escape_pins(self, bbox):
        ...
        # List of pin to new pin name
            ...
            if port in self.write_ports:
                if self.num_spare_cols == 1:
                    pins_to_route.append("spare_wen")
                else:
                    for bit in range(self.num_spare_cols):
                        for port in range(OPTS.num_w_ports):
                            pins_to_route.append("spare_wen{0}[{1}]".format(port, bit))
    ...
    def add_busses(self):
        ...
        if OPTS.RF_mode == False:
            ...
            wen = "w_en"
            pen = "p_en_bar"
            self.control_bus_names.extend([wen, pen])
            self.vert_control_bus_positions = self.create_vertical_bus(layer="m2",
                                                                           pitch=self.m2_pitch,
                                                                           offset=self.vertical_bus_offset,
            ...
            for port in range(OPTS.num_all_ports):
                self.addr_bus_names.append("A{0}[{1}]".format(port, i) for i in range(self.addr_size))
            self.vert_control_bus_positions.update(self.create_vertical_pin_bus(layer="m2",
                                                                                pitch=self.m2_pitch,
                                                                                offset=self.addr_bus_offset,
                                                                                names=self.addr_bus_names,
            ...
            self.bank_sel_bus_names = ["bank_sel0_{1}".format(i) for i in range(self.num_banks)]
            self.vert_control_bus_positions.update(self.create_vertical_pin_bus(layer="m2",
                                                                                pitch=self.m2_pitch,
                                                                                offset=self.bank_sel_bus_offset,
                                                                                names=self.bank_sel_bus_names,
    ...
    def add_modules(self):
        ...
        if OPTS.RF_mode == False:
            ...
            self.data_dff = factory.create("dff_array", module_name="data_dff", rows=1, columns=OPTS.num_w_ports*(self.word_size + self.num_spare_cols))            
        ...
        if OPTS.RF_mode == False:
            ...
            self.control_logic_multiport = self.mod_control_logic(num_rows=self.num_rows,
                                                               words_per_row=self.words_per_row,
                                                               word_size=self.word_size,
                                                               spare_columns=self.num_spare_cols,
                                                               sram=self,
    ...
    def create_col_addr_dff(self):
        ...
        if OPTS.RF_mode == False:
            ...
            insts.append(self.add_inst(name="col_address",
                                           mod=self.col_addr_dff))
            # inputs, outputs/output/bar
            inputs = []
            outputs = []
    ...
    def create_control_logic(self):
        ...
        if OPTS.RF_mode == False:
            for port in self.all_ports:
                ...
                ctrl_insts.append(self.add_inst(name="control{}".format(port), mod=mod))
